...
SELECT 
    city_name,
    ROUND((population * 0.25) / 1000000, 2) AS million_consumer,
    city_rank
FROM
    city
ORDER BY 2 DESC;
...
SELECT 
    i.city_name, SUM(s.total) AS total_revenue
FROM
    sales AS s
        JOIN
    customers AS c ON s.customer_id = c.customer_id
        JOIN
    city AS i ON i.city_id = c.city_id
WHERE
...
(SELECT 
    ...
    DENSE_RANK() OVER(PARTITION BY ci.city_name ORDER BY  count(s.sale_id) DESC) AS r_ank
FROM
    sales AS s
        JOIN
...
SELECT 
    ...
    cr_month_sale,
    last_month_sales,
    ROUND((cr_month_sale - last_month_sales) / last_month_sales * 100,
            2) AS growth_ratio
FROM
    growth_ratio
...
city_rent AS 
( SELECT 
    city_name, estimated_rent, ROUND((population * 0.25)/1000000, 3) as estimated_coffee_consumer_in_millions
FROM
    city)
SELECT 
    cr.city_name,
